...
window.navHTML = (active) => {
  ...
  return `<div class="nav"><div class="wrap row">
    <a class="brand" href="/print"><span class="logo">${ICON.printer(16)}</span><span>${esc(CFG.BRAND||'Peradaban Print')}<small>Layanan cetak ${esc(CFG.PRINTER||'')}</small></span></a>
    <nav class="nav-links">
      ${link('/print/kirim','Cetak','print')}
      ${link('/print/status','Cek Status','status')}
      <a class="cta" href="/print/kirim">Mulai Cetak</a>
    </nav>
  </div></div>`;
};
